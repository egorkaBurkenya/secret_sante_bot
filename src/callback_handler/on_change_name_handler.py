...
@dp.callback_query_handler(text_contains="change_name", state=UserState.Hub.Main)
async def on_change_name_callback_handler(call: CallbackQuery, state: FSMContext):
    ...
    await UserState.Hub.ChangeName.set()

@dp.message_handler(content_types=["text"], state=UserState.Hub.ChangeName)
